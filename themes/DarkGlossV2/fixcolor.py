...
def convert_color(color_string):
  ...
  if color_string.startswith('#'):
    ...
    match = re.match(r'rgba?\(+\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+(?:\.\d+)?)?\s*\)*', color_string)
    match2 = re.match(r'rgba?\(+\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)?(?:\.\d+)?\s*\)*', color_string)
    if match:
      r, g, b, _ = map(int, match2.groups())
      print(color_string)

  elif 'rgb' in color_string:
    match = re.match(r'rgb+\s*?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)', color_string)
    if match:
...
def process_css_file(filename):
  ...
  with open(backup_filename, 'r') as f_in:
    ...
    with open(filename, 'w') as f_out:
      for line in f_in:
        new_line = line
        for match in re.finditer(r'(#[0-9a-f]{6}|#[0-9A-F]{6}|rgb+\s*\(\s*[0-9]+\s*,\s*[0-9]+\s*,\s*[0-9]+\s*\)|rgba+\s*\(\s*[0-9]+\s*,\s*[0-9]+\s*,\s*[0-9]+\s*,\s*[0-9]*.*[0-9]+\s*\))', line):
